...
contract VersionManager is IVersionManager, IModule, BaseFeature, Owned {
    ...
    bytes4 internal constant ADD_MODULE_PREFIX =
        bytes4(keccak256("addModule(address,address)"));
    bytes4 internal constant UPGRADE_WALLET_PREFIX =
        bytes4(keccak256("upgradeWallet(address,uint256)"));
    ...
    function setMinVersion(uint256 _minVersion) external onlyOwner {
        require(
            _minVersion > 0 && _minVersion <= lastVersion,
     ...
     */
    function addVersion(
        address[] calldata _features,
        address[] calldata _featuresToInit
    ) external onlyOwner {
        ...
        for (uint256 i = 0; i < _features.length; i++) {
            ...
            bytes4[] memory sigs =
                IFeature(_features[i]).getStaticCallSignatures();
        ...
        for (uint256 i = 0; i < _featuresToInit.length; i++) {
            require(
                isFeatureInVersion[_featuresToInit[i]][newVersion],
        ...
        returns (bool)
    {
        ...
        return
            _isFeatureAuthorisedForWallet(_wallet, _feature) ||
    ...
    function getRequiredSignatures(
    ...
    ) external pure override returns (uint256, OwnerSignature) {
        ...
        require(
            methodId == UPGRADE_WALLET_PREFIX || methodId == ADD_MODULE_PREFIX,
            "VM: unknown method"
        );
    ...
    function verifyStaticCall() public {
        if (msg.sender != address(this)) {
            ...
            (bool success, ) =
                address(this).call{gas: 3000}(
                    abi.encodeWithSelector(
                        VersionManager(0).verifyStaticCall.selector
                    )
                );
            require(!success, "VM: not in a staticcall");
        } else {
            // second entry in the method (via an external call)
        ...
        onlyWhenUnlocked(_wallet)
    {
        ...
        if (
            _toVersion < minVersion_ &&
            fromVersion == 0 &&
            IWallet(_wallet).modules() == 2
        ) {
        ...
        require(
            toVersion >= minVersion_ && toVersion <= lastVersion,
    ...
    function checkAuthorisedFeatureAndInvokeWallet(
        ...
        uint256 _value,
        bytes memory _data
    ) external override returns (bytes memory _res) {
        require(
            _isFeatureAuthorisedForWallet(_wallet, msg.sender),
    ...
    function _isFeatureAuthorisedForWallet(address _wallet, address _feature)
        private
        view
        returns (bool)
    {
